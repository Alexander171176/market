...
const handleDragEnd = () => {
    const newOrderIds = localValues.value.map(value => value.id)
    emits('update-sort-order', newOrderIds)
}

const toggleAll = (event) => {
    const checked = event.target.checked
    const ids = localValues.value.map(r => r.id)
        ...
        <div class="overflow-x-auto">
            <table v-if="values.length > 0"
                ...
                <tr>
                    ...
                    <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        <div class="font-medium text-left">{{ t('alias') }}</div>
                    </th>
                    <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    ...
                    <template #item="{ element: value }">
                        <tr class="text-sm font-semibold border-b-2
                            ...
                            <td class="px-2 py-2 text-center">
                                <input type="checkbox" :checked="selectedValues.includes(value.id)"
