...
// --- Следим за изменением props.products и обновляем локальную копию ---
...
const handleDragEnd = () => {
    ...
    const newOrderIds = localProducts.value.map(product => product.id);
...
const toggleAll = (event) => {
    ...
    const ids = localProducts.value.map(r => r.id);
                ...
                >
                    <template #item="{ element: product }">
                        <tr class="text-sm font-semibold border-b-2
                            ...
                            <td class="first:pl-5 last:pr-5 py-1">
                                <div class="flex justify-center">
                                    <template v-if="product.images && product.images.length">
                                        <img
                                            :src="getPrimaryImage(product).webp_url || getPrimaryImage(product).url"
                                            :alt="getPrimaryImage(product).alt || t('defaultImageAlt')"
                                            :title="getPrimaryImage(product).caption || t('postImage')"
                                            class="h-6 w-8 object-cover rounded-xs"
                                        >
                                    </template>
                                    <template v-else>
                                        <img
                                            src="/storage/product_images/default-image.png"
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap">
                                <div class="text-center">{{ product.views }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap">
                                <div class="text-center">{{ product.likes_count }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap">
                                <div class="flex justify-center space-x-2">
                                    <LeftToggle :isActive="product.left"
                                                @toggle-left="$emit('toggle-left', product)"
                                                :title="product.left ? t('enabled') : t('disabled')"/>
                                    <MainToggle :isActive="product.main"
                                                @toggle-main="$emit('toggle-main', product)"
                                                :title="product.main ? t('enabled') : t('disabled')"/>
                                    <RightToggle :isActive="product.right"
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap">
                                <div class="flex justify-center space-x-2">
                                    <ActivityToggle :isActive="product.activity"
                                                    @toggle-activity="$emit('toggle-activity', product)"
                                                    :title="product.activity ? t('enabled') : t('disabled')"/>
                                    <CloneIconButton @clone="$emit('clone', product)"/>
                                    <IconEdit :href="route('admin.products.edit', product.id)" />
                                    <DeleteIconButton @delete="$emit('delete', product.id)"/>
                                </div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-1 whitespace-nowrap">
                                <div class="text-center">
