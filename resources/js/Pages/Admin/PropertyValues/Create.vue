...
/**
 * Форма создания значения характеристики
 */
const form = useForm({
    activity: false,
    locale: '',        // 'ru' | 'en' | 'kk' ...
...
const submit = () => {
    ...
    form.post(route('admin.property-values.store'), {
        errorBag: 'createPropertyValue',
        preserveScroll: true,
        onSuccess: () => {
            toast.success('Значение успешно создано!')
        },
        onError: (errors) => {
            console.error("❌ Ошибка при созданиизначения характеристики:", errors);
...
<template>
    <AdminLayout :title="t('createPropertyValue')">
        <template #header>
            <TitlePage>{{ t('createPropertyValue') }}</TitlePage>
        </template>
